...
[['text'=>'تفعيل التوجيه ' ,'callback_data'=>"🌐🌐"],['text'=>'قفل التوجيه ' ,'callback_data'=>"🌐🌐"]],
...
'text'=>"عزرا بوتات الويبهوك لا تدعم هذه الميزه ⚙️",
 'show_alert'=>true,
]);} 
if($data== 'GGTR'){
bot('EditMessageText',[
...
$ok = json_decode(file_get_contents("ok.json"),1);
if($text == "/start" or $text == "• رجوع"){
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"• اهلا بك في بوت ويبهوك ",
	"reply_markup"=>json_encode([
	'keyboard'=>[
	[["text"=>"• عمل ويب هوك"],["text"=>"• حذف ويب هوك والتحديثات"]],
	[["text"=>"• معلومات التوكن"]],
	],'resize_keyboard'=>true,
	])
	]);
$ok["$chat_id"] = "done";	
file_put_contents("ok.json",json_encode($ok,128|34|256));
}
	
	
	
	if($text and $ok["$chat_id"] == "getlink"){
		file_get_contents("https://api.telegram.org/bot".$ok["tok"][$chat_id]."/deletewebhook&drop_pending_updates=true");
		file_get_contents("https://api.telegram.org/bot".$ok["tok"][$chat_id]."/setwebhook?url=$text");
		bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"• تم الانشاء بنجاح وحشف الطلبات المعلقة✅️ ",
	"reply_markup"=>json_encode([
	'keyboard'=>[
	[["text"=>"• رجوع"]],
	],'resize_keyboard'=>true,
	...
	if($text and $ok["$chat_id"] == "gettok"){
	$js = json_decode(file_get_contents("https://api.telegram.org/bot$text/getwebhookinfo"),1);
	$url = $js["result"]["url"];
	$pend = $js["result"]["pending_update_count"];
	$max = $js["result"]["max_connections"];
$ip = $js["result"]["ip_address"];
$js2 = json_decode(file_get_contents("https://api.telegram.org/bot".$text."/getme"),1);
$us = $js2["result"]["username"];
$na = $js2["result"]["first_name"];
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"
• البوت مربوط مع : $url
• الطلبات المعلقة : $pend
• اقصى عدد طلبات مسموح بلثانية : $max
• الايبي : $ip
• معرف البوت : @$us
• اسم البوت : $na
• ارسل رابط الموقع https:// ",
	"reply_markup"=>json_encode([
	'keyboard'=>[
	[["text"=>"• رجوع"]],
	],'resize_keyboard'=>true,
...
• ارسل توكن البوت",
	...
	if($text and $ok["$chat_id"] == "okdel"){
		file_get_contents("https://api.telegram.org/bot".$text."/deletewebhook&drop_pending_updates=true");
		bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"• تم الحذف وحذف الطلبات المعلقة✅️",
	"reply_markup"=>json_encode([
	'keyboard'=>[
	[["text"=>"• رجوع"]],
	],'resize_keyboard'=>true,
	...
	if($text == "• حذف ويب هوك والتحديثات"){
		bot('sendmessage',[
	'chat_id'=>$chat_id,
